...
impl GPlayer {
    ...
    pub async fn enter(gplayer: GPlayer, pool: &Pool<PostgresConnectionManager<tokio_postgres::NoTls>>) -> Result<i64, RunError<tokio_postgres::Error>> {
      ...
      let stmt = conn.prepare("INSERT INTO public.\"gplayer\" (game_id, user_id, enter_timestamp) VALUES ($1, $2, $3) RETURNING id;").await?;
      let row = conn.query_one(&stmt, 
                  &[&gplayer.game_id, &gplayer.user_id, &gplayer.enter_timestamp]).await?;
